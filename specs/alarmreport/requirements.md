...
2. **Thu thập dữ liệu (Data Gathering)**: Truy xuất vào Driver SCADA (`iDriver`) để đọc giá trị (Value) của 18 thanh ghi theo cấu hình.
...
- Cấu trúc cột sẽ map cứng với file Excel thực tế, bao gồm: `ID`, `DateTime`, `TagNo`, `QuyTrinh`, `CongDoanMay`, cùng với tất cả 18 thanh ghi thời gian và nhiệt độ (như `ThoiGianCapLieu`, `ThoiGianTron1`, `NhietDoMay`...).
...
- **Đang chạy**: Suốt quá trình từ công đoạn 1 đến 5, Timer cứ +30s là đọc toàn bộ 18 thanh ghi và `INSERT` 1 dòng vào database.
...
- Cần có trường `DeviceName` (hoặc `MachineName`) trong database lưu giá trị này (ví dụ: `"AFChemTX01"`). Nhờ vậy, nếu sau này nhà máy có thêm cụm máy khác (ví dụ "AFChemTX012"), dữ liệu báo cáo 18 thanh ghi sẽ được ghim chính xác cho từng cỗ máy dựa vào tên Device này, rất rõ ràng và chuẩn xác.
