...
1. **[NEW] WebhookHttpServer.cs** (`HinoTools.Data/Http/WebhookHttpServer.cs`):
   ...
   - Triển khai cơ chế xác thực Token bí mật thông qua header `X-Webhook-Token`.
...
3. **[MODIFY] AlarmReportLogger.cs** (`HinoTools.Data/Log/AlarmReportLogger.cs`):
   ...
   - Bổ sung các thuộc tính cấu hình trực quan trong Category `"Hino Settings"`:
     ...
     - `WebhookToken` (Mã bảo mật Webhook, mặc định `HinoWebhookSecretToken2026`).
...
<appSettings>
  ...
  <add key="WebhookToken" value="HinoWebhookSecretToken2026" />
</appSettings>
```

### 3.2. Cung cấp API Endpoint cho Bên thứ ba
Hệ thống Base (bên thứ ba) sẽ gắn Webhook của bạn vào hệ thống của họ với thông tin sau:
- **API Endpoint URL**: `http://[IP_MAY_CHU]:5600/api/webhook`
- **HTTP Method**: `POST`
- **HTTP Headers bắt buộc**:
  - `Content-Type`: `application/json`
  - `X-Webhook-Token`: `HinoWebhookSecretToken2026` (Khớp với cấu hình của bạn)
...
Gửi một yêu cầu POST hợp lệ:
```bash
curl -X POST http://localhost:5600/api/webhook \
  -H "Content-Type: application/json" \
  -H "X-Webhook-Token: HinoWebhookSecretToken2026" \
...
3. **Sai Content-Type**:
   - Thay đổi header `-H "Content-Type: text/plain"`.
   - *Kỳ vọng phản hồi*: HTTP Status `415 Unsupported Media Type`.
